...
resource "aws_lambda_function" "testRequest" {
  function_name = "TestRequest"

  s3_bucket = aws_s3_bucket.lambda_bucket.id
  s3_key = aws_s3_object.lambdas.key

  runtime = "nodejs12.x"
  handler = "testRequest.handler"

  source_code_hash = data.archive_file.lambdas.output_base64sha256
  role = aws_iam_role.lambda_exec.arn
}

resource "aws_cloudwatch_log_group" "testRequest" {
  name = "/aws/lambda/${aws_lambda_function.testRequest.function_name}"
  retention_in_days = 30
}
...
resource "aws_apigatewayv2_integration" "testRequest" {
  api_id = aws_apigatewayv2_api.lambda.id

  integration_uri    = aws_lambda_function.testRequest.invoke_arn
  integration_type   = "AWS_PROXY"
  integration_method = "POST"
}

resource "aws_apigatewayv2_route" "testRequest" {
  api_id = aws_apigatewayv2_api.lambda.id

  route_key = "GET /testRequest"
  target    = "integrations/${aws_apigatewayv2_integration.testRequest.id}"
}
...
resource "aws_lambda_permission" "api_gw_testRequest" {
  statement_id  = "AllowExecutionFromAPIGateway"
  action        = "lambda:InvokeFunction"
  function_name = aws_lambda_function.testRequest.function_name
  principal     = "apigateway.amazonaws.com"
